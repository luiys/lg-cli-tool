...
export class Commands {
	...
	static initializeGit() {
		...
		else return { flagErro: false, result: `Git inicializado com sucesso` };
	}
	...
	static cloneRepo(repo: string) {
		...
		else return { flagErro: false, result: `Repositório ${repo} clonado com sucesso` };
	}

	static navigateTo(dir: string) {
		shell.cd(dir)
		if (!!shell.error()) return { flagErro: true, result: `Erro ao navegar para ${dir}` };
		else return { flagErro: false, result: `Navegado para ${dir}` };
	}
	...
	static installDeps() {
		...
		else return { flagErro: false, result: `Dependências instaladas com sucesso` };
	}
	...
	static renameDir(oldName: string, newName: string) {
		...
		else return { flagErro: false, result: `Arquivo ${oldName} renomeado com sucesso para ${newName}` };
	}
	...
	static moveDir(origin: string, destiny: string) {
		...
		else return { flagErro: false, result: `Arquivo ${origin} copiado com sucesso para ${destiny}` };
	}
	...
	static deleteDir(dir: string) {
		...
		else return { flagErro: false, result: `Diretório ${dir} removido com sucesso` };
	}
